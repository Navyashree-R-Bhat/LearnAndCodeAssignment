...
bool Client::authenticateUser(const std::string &userId, const std::string &password)
{
    bool status = false;
    std::string request = "VALIDATE:" + userId + ":" + password;
...
void Client::adminDisplayScreen()
{
    ...
    if(authenticateUser(userId, password) && userId == "ADMIN_1")
    {
        int choice;
